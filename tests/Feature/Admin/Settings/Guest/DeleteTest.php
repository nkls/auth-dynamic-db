...
class DeleteTest extends TestCase
{
    ...
    {
        $response = $this->delete(route('settings.guest.delete'));
    ...
    public function test_delete_settings()
    {
        $response = $this->withTokenByUUID(static::UUID)
            ->delete(route('settings.guest.delete'));

        $response->assertOk();
        $this->assertDatabaseMissing(Setting::class, ['param' => 'guest']);
    }
    ...
    public function test_access_denied_for_role_user()
    {
        ...
        $response = $this->withTokenByUUID(static::UUID)
            ->get(route('settings.guest.delete'));

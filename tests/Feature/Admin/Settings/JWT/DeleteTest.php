...
class DeleteTest extends TestCase
{
    ...
    {
        $response = $this->delete(route('settings.jwt.delete'));

        $response->assertStatus(401);
    }

    public function test_delete_jwt_settings()
    {
        //Setting::where('param', 'jwt')->delete();

        $response = $this->withTokenByUUID(static::UUID)
            ->delete(route('settings.jwt.delete'));

        $response->assertOk();
        $this->assertDatabaseMissing(Setting::class, ['param' => 'jwt']);
    }

    public function test_error_delete_jwt_settings()
    {
        Setting::where('param', 'jwt')->delete();
